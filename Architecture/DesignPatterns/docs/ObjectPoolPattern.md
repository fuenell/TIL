...
유니티에서 오브젝트를 미리 생성해둔 뒤 새로운 오브젝트가 필요할 때 미리 생성해둔 오브젝트를 사용하고 해제하여

런타임 생성/삭제 성능을 개선 시키는 방법

# 간이 풀링 방법
간이 풀링 방법으로 최대로 생성될 개수를 미리 알 수 있다면 그것보다 많이 생성해두면 굳이 해제를 일일이 체크하지 않고 계속 사이클을 돌릴 수 있다. (ex.10*10 오목판에 생성될 수 있는 최대 돌은 100개), 프로토타이핑와 같은 간단히 구현하고 싶을 때 사용해볼 수 있다.
...
# 정규적인 방법
할당 시 PoolList에서 꺼내서 사용하고,
사용이 끝난 오브젝트는 스스로 Release를 호출하여 다시 PoolList에 들어간다.

### IObjectPool

직접 구현해도 되지만 Unity에서 이미 제공해주는 `IObjectPool` 인터페이스를 활용하면 빠르게 구현이 가능하다.
